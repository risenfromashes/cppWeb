...
/*! Returns the standard HTTP status message.
 * \param code An HTTP status code.
 * \return The standard HTTP status message for the given \p code or an empty \c std::string()
 * if no standard phrase for the given \p code is known.
 */
inline std::string status(Code code)
{
    return std::to_string((int)code) + " " + reasonPhrase((int)code);
}
